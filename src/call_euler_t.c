...
/*  --- experimental version with transposed yout                       --- */
/*==========================================================================*/

#include "rk_util.h"

SEXP call_euler_t(SEXP Xstart, SEXP Times, SEXP Func, SEXP Initfunc,
  ...
  for (it = 0; it < nt - 1; it++) {
    ...
    if (it < nt) {
      yout[ntot * it] = t + dt;
      for (i = 0; i < neq; i++) yout[ntot * it + 1 + i] = y0[i];
    }
  } /* end of main loop */
  /*------------------------------------------------------------------------*/
